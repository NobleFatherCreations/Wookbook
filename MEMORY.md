...
## Update (2026-08-25, session 16 continued) — registry audit: why things read as "no info available"

User asked, before more adaptation work, for an audit of the discrepancy
class that produced the Pattern Decoder's phantom "no source." There were
six more, all the same shape: **a lookup returns nothing, so the answer looks
like "no info available" when the information is on disk.**

**1. `codename` was null for four of twelve projects** — The Loop, The
Weighing, the Codex, The Listening Room — in both `sites.json` and
`chapters.json`. Anything keyed on codename could not see a third of the
catalogue, including the two flagship books. Filled from `slug`.

**2. Four competing identifier vocabularies.** `sites.json` has `codename`
AND `slug`; `chapters.json` keys on `codename`; `BOOKS.md` headings use
`slug`; `design/extract-prose-master.py` had a fourth set. Only `fractal` had
the same value in all of them. **`slug` is now canonical everywhere** — it
matches the live URL and the BOOKS.md heading, so it is the name a person
already knows. `codename` survives as an alias only. `festie+codex` and
`festie+bible` were not filename-safe; now `festie-codex` / `festie-bible`.

**3. The wook blocker was false, and had been for three weeks.** BOOKS.md
carried "unresolved discrepancy — diff before doing any design work here."
`festie-codex-full.html` and `source/projects/noble-father-festival.html` are
**byte-identical** (md5 `e917c570…`). Worse: `sites.json` →
`projects[wook].localSourceNote` already said **RESOLVED 2026-08-03** — while
`sites.json` → `notes[6]` in the *same file* still said unresolved. The title
confusion was with a THIRD file, `noble-father-festiebible.html`, a different
project. All three stale copies rewritten.

**4. Both duplicate pairs are intentional and are now enforced rather than
described.** wook (repo-root file is what Netlify builds; source/projects is
what sweeps walk) and music (both generated by `build-music.py`). They are
declared in `sites.json` → `syncPairs`, and the audit **fails when they
differ** — the wook pair silently drifted once already on 2026-08-05. The
check is deliberately inverted from the obvious one: identical is required,
drift is the bug.

**5. The Festie Bible was the least findable thing in the repo.** A v6
project, 12 structured role-based guides, and it had *no* BOOKS.md section,
*no* chapters.json record and *no* prose extraction — reachable only by
guessing a filename. Now has a full BOOKS.md section and a chapters.json
record. Its data (`content/festie-bible-data.json`, 12 guides: grove, bass,
rave, pride, create, sound, market, hold, care, lead, event, safe — each with
checks/outline/sentences/scenarios) was fine all along. **Its extractor is
not re-runnable** — `design/extract-festie-bible.py` hardcodes a dead
scratchpad OCR path and `/home/user/Wookbook/` with a capital W.

**6. `content/prose/` filenames collided with a slug.** `festival.md` held
The Festie *Codex* while slug `festival` belongs to The Festie *Bible*. That
is the mechanism by which the two were confused. Renamed to slugs:
`festival`→`wook`, `sovereign`→`feminine`, `playground`→`children`,
`root`→`shadowroot`. The extractor was updated so this stays reproducible,
and it also still carried the stale title "The Fracture Everywhere" (renamed
2026-08-12).

**7. `fixes/faith.html` was tracked but referenced nowhere.** A byte-verified
leak fix awaiting redeploy that nothing could find. Registered on the faith
project as `leakFix`.

**Two new durable guards, both generated, neither hand-maintained:**

- `design/audit-registry.py` — cross-checks all three registries against each
  other and against disk; exits non-zero. Every check exists because one of
  the above actually happened. Run it before any deploy.
- `design/build-index.py` → **`INDEX.md`** — one page answering "where is X?"
  for any project: canonical slug, every alias, live URL, page source,
  structured data, prose, chapter data, notes, pending fixes, sync pairs.
  Generated from `sites.json`, so it cannot drift.

**Audit now passes with 0 failures.** The 13 remaining warnings are real work,
not defects: 8 books have no chapter data in `chapters.json` (the prose exists
for all of them, so this is a formatting job, not a recovery job), all 85
Loop/Weighing chapters have null slugs, and the Festie Bible has no prose
extraction.
