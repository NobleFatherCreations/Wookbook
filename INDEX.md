...
**Generated. Do not hand-edit** — run `python3 design/build-index.py --apply` after any `sites.json` change. `design/audit-registry.py` enforces the rules this relies on and exits non-zero when they break.
...
| Project | slug | alias | Live | Prose | Words |
|---|---|---|---|---|---|
...
*Honor the faith · name the machinery*
...
- **chapter data** — `chapters.json` → `faith`
- **notes** — `BOOKS.md` → `## faith`
- **pending fix** — `fixes/faith.html`

### The Festie Codex

*Festival & field harm reduction*
...
- **chapter data** — `chapters.json` → `wook`
- **notes** — `BOOKS.md` → `## wook`
- **kept in sync with** — `festie-codex-full.html` + `source/projects/noble-father-festival.html`

### The Fracture

*The wealth transfer · 195 citations*
...
*The architecture, 29 sectors wide*
...
*42 chapters · recovery for women*
...
- **slug** — `loop`
- **alias (`codename`)** — none — same as slug
- **live** — https://noblefathercreations.com/loop
- **version** — v2
- **shape** — 47 chapters in 8 movements
- **page source** — `fixes/loop.html`
- **structured data** — —
- **prose** — `content/prose/loop.md` (25,736 words)
- **chapter data** — `chapters.json` → `loop`
- **notes** — `BOOKS.md` → `## loop`
...
- **slug** — `scale`
- **alias (`codename`)** — none — same as slug
- **live** — https://noblefathercreations.com/scale
- **version** — v2
- **shape** — 38 chapters in 6 movements
- **page source** — `fixes/scale.html`
- **structured data** — —
- **prose** — `content/prose/scale.md` (18,791 words)
- **chapter data** — `chapters.json` → `scale`
- **notes** — `BOOKS.md` → `## scale`

### The Pattern Decoder

*349 tactics · type what happened*
...
- **chapter data** — `chapters.json` → `playbook`
- **notes** — `BOOKS.md` → `## playbook`
...
- **chapter data** — `chapters.json` → `shadowroot`
- **notes** — `BOOKS.md` → `## shadowroot`

### The Listening Room

*Sorted by what each song is for*

- **slug** — `music`
- **alias (`codename`)** — none — same as slug
- **live** — https://noblefathercreations.com/music
- **version** — v2
- **shape** — 1 sections, 0 sub-entries
- **page source** — `source/projects/noble-father-music.html`
- **structured data** — —
- **prose** — `content/prose/music.md` (80 words)
...
Short-form work products, as they get built:
...
| `playbook` | `content/decoder-clips.json` | 349 five-beat clip records; 112 shoot-ready, 152 held, 85 awaiting a hook |
| `playbook` | `HOOKS-TODO.md` | worksheet: candidate opening lines for the 215, blocked categories first |
| `playbook` | `content/recipe-clips.json` | 109 tactic sequences; 79 shoot-ready, 30 held on their own steps' grading |
| `playbook` | `CODEX-TODO.md` | 266 entries whose why_this_matters or what_it_is_not is still scaffolding |
| `festival` | `content/festie-bible-data.json` | 12 role-based field guides with checks, outlines and scenarios |
| `festival` | `content/bible-clips.json` | 149 scenarios, all shoot-ready; 127 cross-link to Decoder tactics |
...
| File | Holds | Keyed on |
|---|---|---|
| `sites.json` | live URL, deploy route, sources, versions, changelog, sync pairs | `slug` |
| `BOOKS.md` | per-book content, stance and design position | `slug` (heading) |
| `chapters.json` | movement/chapter data | `slug`, with `project` as alias |
| `MEMORY.md` | cross-session history, append-only | prose |
| `INDEX.md` | this file — where everything is | generated |
